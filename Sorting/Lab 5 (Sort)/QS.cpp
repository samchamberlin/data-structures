...
void QS::sort(int left, int right){
    if(right == left || left < 0 || right > currentSize - 1 || left > right){
        return;
    }
...
void QS::swap(int value1, int value2){
    int temp1 = myArray[value1];
    myArray[value1] = myArray[value2];
    myArray[value2] = temp1;
}
...
int QS::medianOfThree(int left, int right){
    ...
    if(myArray == NULL || currentSize < 0 || left > right || right < left || left < 0 || right > currentSize - 1 || left == right){
        return -1;
    }
...
int QS::partition(int left, int right, int pivotIndex){
    if(myArray == NULL || currentSize < 0 || maxSize <= 0 || left < 0 || right < 0 || !(left < right) ||
       right > currentSize - 1 || !(pivotIndex <= right) || !(pivotIndex >= left)){
        return -1;
    }
